...
export {
    ...
    "correctionVector","allFittingIdeals","isVectorBundle","generateHomogeneousModule",
    "infoInitialModules","listDegenerations","analyzeDegenerations",
    "saveDegeneration","specialSubfan", "fanToString",
    -- Methods inputs:
    "Mode","Random","Correction","TimeSpent","VectorBundle",
    -- Hash Table Keys:
...
-- Aditional exports
--export {"allFittingIdeals", "raysProperty", "byCones", "latticeInteriorPoint","byCones","raysProperty","byDimension"};
...
correctionVector = method( Options => {  });
...
--auxiliary function to reduce the code duplicity in the next functions
auxInfoModules = method(Options => {Mode => "radical", Correction =>{}});

auxInfoModules(Module, Fan) := opts -> (M,F) ->(
  n := numgens(ring M);
  m := rank (ambient M);
  rs:= rays F;
  Lh :={};
  ...
  for i from dim(source linealitySpace(F)) to dim(F) do(
      ...
      Lh = join(Lh, apply(lr, h -> (h#intPoint = latticeInteriorPoint( polyhedron posHull(h#cone));
       h#module = initialModule(M, take(h#intPoint,{0,n-1}), take(h#intPoint,{n,n+m-1}), Correction => opts#Correction) ; h#quotient = (coker gens h#module) ;  h)));
    );
...
infoInitialModules(Module, Fan, Ideal) := opts ->(M,F,I)-> (
    ...
    -- Check that the module is reflexive
    ...
    if opts#VectorBundle === "true" then(
        if r === false then( Lh = apply (Lh, h-> (h#isVectorBundle= false; h)); )
        else(
            r= r_1;
...
-- TO DO: there is code in common with the previous function, I should try to merge them
...
doc ///
  ...
  Description
    Text
      In order to compute the initial module with respect to some weights, we need this weights
      to be positive integers, and so correctionVector computes a positive vector in the lineality space of a fan.
      This computation can be performed on the fan, the matrix corresponding to the lineality space of said fan
      or directly on the module.
    Example
...
doc ///
   ...
   Inputs 
      ...
      s: List
      Correction => List
        a vector with positive entries in the lineality space of the Grobner fan of M
      TimeSpent => ZZ
   ...
   Description 
	Text 
	  ...
	  the terms of lowest weight.  There is an unchecked assumption that the module is homogeneous.
          The computation needs the weight vector to be positive and to stay in the same cone in the Grobner fan of the module.
          A correction vector is computed automatically, but it can be also provided explicitly using Correction.
          The maximum time for the computation is set to be 30 seconds, returning the zero module if the time is exceded.
          In order to change the time limit use TimeSpent to indicate the maximun time in seconds spent in the computation.
	Example
	   S=QQ[x,y];
...
doc ///
  ...
  Inputs
    ...
    Correction => List
      specifies the correction vector used to compute the initial modules
  Outputs
    L:List
  Description
    Text
      The function takes an homogeneous submodule M of a free module oplus S e_i, where S is a graded module (the Cox ring of a toric varietay), 
      the Grobner fan F of M, and the irrelevant ideal I of the toric variety. The output is a list of hash tables with the information about the degenerations corresponding to the cones in the Grobner fan of M. 
      The list contains as its first element the Grobner fan F of the module and the remaining elements are MutableHashTables, one for each cone in F. 
      ...
      The degeneration considered in each cone is the quotient of the initial module of each cone, that is, this function studies degenerations on S^m/M. 

      The keys of these tables store information such as whether the degeneration is a vector bundle, torsion-free, reflexive or equivariant.  
      It also stores the infomation about the cone, the point used to compute the initial module and the corresponding initial module.
      New keys can be added to the hash tables by the user if they wish.
      
      WARNING: for some examples the computation of the radical of the fitting ideals or the saturation with respect to the ideal I can abbort, use VectorBundle => "false" to skip this computation.
    Example
      S= QQ[x,y,z,Degrees => {{2},{1},{2}}];
      I= ideal(x,y,z);
      M= image map(S^{{-4}, {0}}, S^{{-6}}, matrix {{y^2+z}, {x^3+ y^6}});
...
doc ///
  ...
  Description
    Text
      Given a module M, it computes the Minkowski sum of the Newton polytopes of the generators of M
      and returns the normal fan of this polytope.  This can be used as a quick approximation of the Grobner fan of M if the computation of the Grobner fan takes too long.
      Moreover, if the module is monogenerated it coincides with the Grobner fan of M.
    Example
      S= QQ[x,y,z,Degrees => {{2},{1},{2}}];
      I= ideal(x,y,z);
      M= image map(S^{{-4}, {0}}, S^{{-6}}, matrix {{y^2+z}, {x^3+ y^6}});
...
doc ///
  ...
  Description
    Text
      ...
      This code is part of the saveDegeneration function, but it can be used separately to have executable code for the fan.
    Example
      S= QQ[x,y,z,Degrees => {{2},{1},{2}}];
      I= ideal(x,y,z);
      M= image map(S^{{-4}, {0}}, S^{{-6}}, matrix {{y^2+z}, {x^3+ y^6}});
...
doc ///
  Key
    allFittingIdeals
  Headline
    lists all the fitting ideals of a module
  Usage
    L = allFittingIdeals(M)
  Inputs
    M:Module
  Outputs
    L:List
  Description
    Text
      Given a module M, this function returns the list of all fitting ideals of M, that is, the fitting ideals of order 0 to rank ambient M +1.
    Example
      S= QQ[x,y,z,Degrees => {{2},{1},{2}}];
      I= ideal(x,y,z);
      M= image map(S^{{-4}, {0}}, S^{{-6}}, matrix {{y^2+z}, {x^3+ y^6}});
...
doc ///
  Key
    specialSubfan
  Headline
    creates the subfan of F given by a list modules
  Usage
    F = specialSubfan(F, LAM)
  Inputs
    F:Fan
    LAM:List
      a list of hash tables with keys coneId and cone, as in the output of infoInitialModules
  Outputs
    F:Fan
  Description
    Text
      Given a fan F, this function returns the subfan of F, the Grobner fan of a module, given by a specific list of modules.
      The typical input is the list of vector bundles obtained from listDegenerations under the key vectorBundles;
      or the list of non-equivariant modules, under the key notEquivariants.
      It assumes that the input list yields a valid subfan. 
///
